    ...
    def tx_pkt(self):
        ...
        try:
            ...
            if len(pkt_S) > self.mtu:
                ...
                packet = network.NetworkPacket.from_byte_S(pkt_S)

                # Segment it
                while len(packet) > self.mtu:
                    pkt_S = packet.segment(len(packet) - self.mtu).to_byte_S()
                    self.out_intf.put(pkt_S)
                    print('%s: transmitting packet segment "%s"' % (self, pkt_S))

                # Transmit the final segment
                pkt_S = packet.to_byte_S()
                self.out_intf.put(pkt_S)
                print('%s: transmitting final packet segment "%s"' % (self, pkt_S))
            else:
                self.out_intf.put(pkt_S)
